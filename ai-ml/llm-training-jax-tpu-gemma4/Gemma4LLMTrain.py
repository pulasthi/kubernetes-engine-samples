...
def run_training(maxlen, batch_size, datacount):
    ...
    # Get the Gemma4 model
    model = gm.nn.Gemma4_E2B(text_only=True)
    # Load the pretrained parameters
    params = gm.ckpts.load_params(gm.ckpts.CheckpointPath.GEMMA4_E2B_PT)
    params = jax.tree.map(lambda x: x.astype(jnp.bfloat16) if jnp.issubdtype(x.dtype, jnp.floating) else x, params)
    # Create an optimizer
